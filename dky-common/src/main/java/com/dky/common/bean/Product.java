package com.dky.common.bean;

import java.math.BigDecimal;
import java.util.Date;

public class Product extends PojoObjectBase {
    private Long id;

    private Long adClientId;

    private Long adOrgId;

    private String isactive;

    private Date creationdate;

    private Long ownerid;

    private Date modifieddate;

    private Long modifierid;

    private Long cUomId;

    private String name;

    private String value;

    private Long mSizegroupId;

    private Long mAttributesetId;

    private String imageurl;

    private Long mDim1Id;

    private Long mDim2Id;

    private Long mDim3Id;

    private Long mDim4Id;

    private Long mDim5Id;

    private Long mDim6Id;

    private Long mDim7Id;

    private Long mDim8Id;

    private Long mDim9Id;

    private Long mDim10Id;

    private String serialno;

    private String proarea;

    private String batchno;

    private String flowno;

    private String unit;

    private String documentnote;

    private BigDecimal precost;

    private BigDecimal pricelist;

    private String help;

    private String description;

    private String descriptionurl;

    private String versionno;

    private Long mDim11Id;

    private Long mDim12Id;

    private String isfairpro;

    private Long mDim13Id;

    private Long mDim14Id;

    private Long mDim15Id;

    private String fairpdttype;

    private String colors;

    private String sizes;

    private String intscode;

    private Date statustime;

    private Long statuserid;

    private Long auPiId;

    private String auState;

    private Long mDim16Id;

    private Long mDim17Id;

    private Long mDim18Id;

    private Long mDim19Id;

    private Long mDim20Id;

    private String addsdesc;

    private String shortname;

    private String stepcode;

    private String datasource;

    private String fabcode;

    private String fabelement;

    private BigDecimal weight;

    private String generated;

    private Short status;

    private String protype;

    private String mainfabremark;

    private String withfabremark;

    private Long yFactoryId;

    private Integer fabdate;

    private Integer clothdate;

    private Integer otcdate;

    private String styledescription;

    private String badicon;

    private String craftworkimg;

    private String packstyle;

    private String tailornotice;

    private String metricimg;

    private String craftworkimgtwo;

    private String bathmode;

    private Long yStyleBomId;

    private String craftworkimgthree;

    private Long yProofId;

    private String confirmcomment;

    private BigDecimal pricelist2;

    private String standard;

    private String composition;

    private String batharea;

    private String quality;

    private Long yBathmodeId1;

    private Long yBathmodeId2;

    private Long yBathmodeId3;

    private Long yBathmodeId4;

    private Long yBathmodeId5;

    private Long yBathmodeId6;

    private Long yBathmodeId7;

    private Long suggestmoq;

    private Long bCodeFormatId;

    private Long boxQty;

    private Long cSupplierId;

    private Long mColorId;

    private BigDecimal acost;

    private BigDecimal pricedis;

    private BigDecimal priceSug;

    private BigDecimal origcost;

    private Short transstatus;

    private String eName;

    private Integer marketdate;

    private String isspecial;

    private String categorymark;

    private String catalogno;

    private String description2;

    private String clothes;

    private String isinternal;

    private String pdtPairup;

    private Long cCurrencyId;

    private BigDecimal pricemember;

    private String matchrule;

    private String isgift;

    private String virtualname;

    private BigDecimal lowestDiscount;

    private Long mDim21Id;

    private String isagt;

    private Integer agtdate;

    private String impStand;

    private String colorsalias;

    private BigDecimal yPriceActual;

    private String isteusclr;

    private String selfwork;

    private String pdtType;

    private Long mDimNew1Id;

    private Long mDimNew2Id;

    private Long mDimNew3Id;

    private Long mDimNew4Id;

    private Long mDimNew5Id;

    private Long mDimNew6Id;

    private Long mDimNew7Id;

    private Long mDimNew8Id;

    private Long mDimNew9Id;

    private Long mDimNew10Id;

    private Long mDimNew11Id;

    private Long mDimNew12Id;

    private Long mDimNew13Id;

    private Long mDimNew14Id;

    private Long mDimNew15Id;

    private Long mDimNew16Id;

    private Long mDimNew17Id;

    private Long mDimNew18Id;

    private Long mDimNew19Id;

    private Long mDimNew20Id;

    private Long mDimNew21Id;

    private Long mDimNew22Id;

    private Long mDimNew23Id;

    private Long mDimNew24Id;

    private Long mDimNew25Id;

    private Long mDimNew26Id;

    private Long mDimNew27Id;

    private Long mDimNew28Id;

    private Long mDimNew29Id;

    private Long mDimNew30Id;

    private Long mDimNew31Id;

    private Long mDimNew32Id;

    private Long mDimNew33Id;

    private Long mDimNew34Id;

    private Long mDimNew35Id;

    private Long mDimNew36Id;

    private Long mDimNew37Id;

    private Long mDimNew38Id;

    private Long mDimNew39Id;

    private Long mDimNew40Id;

    private Long mDimNew41Id;

    private String xwValue;

    private String ycValue;

    private BigDecimal jkValue;

    private BigDecimal xcValue;

    private BigDecimal lbccValue;

    private BigDecimal xbccValue;

    private BigDecimal xkccValue;

    private String dkValue;

    private BigDecimal dkslValue;

    private BigDecimal hzxcValue;

    private BigDecimal dc1Value;

    private BigDecimal dcValue;

    private BigDecimal mjkValue;

    private BigDecimal mjcValue;

    private String isRetail;

    private String notlimitdis;

    private String isMscode;

    private String colorarrDes;

    private String dkNumber;

    private String colorarr;

    private Long colorvalue;

    private String amjValue;

    private String qtmjValue;

    private BigDecimal hzxc1Value;

    private String qtxkValue;

    private String jzValue;

    private String qtxbValue;

    private String tangz;

    private String huax;

    private String lxsx2Value;

    private String lxsx1Value;

    private String qtlbValue;

    private String lxsx3Value;

    private String lxsx4Value;

    private String lxsx5Value;

    private String lingNumber2Value;

    private String lingNumber1Value;

    private String lingValue;

    private String qtxbzzValue;

    private BigDecimal xbcValue;

    private BigDecimal lingCcValue;

    private String xbzzValue;

    private String qtjxValue;

    private String qtxxValue;

    private String xbValue;

    private String sizeType;

    private BigDecimal price;

    private BigDecimal price1;

    private BigDecimal jlPrice;

    private Long approveId;

    private BigDecimal retailPrice;

    private BigDecimal retailPrice1;

    private String mptbelongtype;

    private Long mBasemptsizeId;

    private String xwoldValue;

    private String ycoldValue;

    private String jkoldValue;

    private String xcoldValue;

    private String showpic;

    private String czDate;

    private String no;

    private String customer;

    private String description3;

    private String description5;

    private String description4;

    private String description6;

    private String description7;

    private Long cCustomerId1;

    private Long mDim22Id;

    private Long toStoreid;

    private BigDecimal actualprice;

    private String isRecommend;

    private String code;

    public String getCode() {
        return code;
    }

    public void setCode(String code) {
        this.code = code;
    }

    public Long getId() {
        return id;
    }

    public void setId(Long id) {
        this.id = id;
    }

    public Long getAdClientId() {
        return adClientId;
    }

    public void setAdClientId(Long adClientId) {
        this.adClientId = adClientId;
    }

    public Long getAdOrgId() {
        return adOrgId;
    }

    public void setAdOrgId(Long adOrgId) {
        this.adOrgId = adOrgId;
    }

    public String getIsactive() {
        return isactive;
    }

    public void setIsactive(String isactive) {
        this.isactive = isactive == null ? null : isactive.trim();
    }

    public Date getCreationdate() {
        return creationdate;
    }

    public void setCreationdate(Date creationdate) {
        this.creationdate = creationdate;
    }

    public Long getOwnerid() {
        return ownerid;
    }

    public void setOwnerid(Long ownerid) {
        this.ownerid = ownerid;
    }

    public Date getModifieddate() {
        return modifieddate;
    }

    public void setModifieddate(Date modifieddate) {
        this.modifieddate = modifieddate;
    }

    public Long getModifierid() {
        return modifierid;
    }

    public void setModifierid(Long modifierid) {
        this.modifierid = modifierid;
    }

    public Long getcUomId() {
        return cUomId;
    }

    public void setcUomId(Long cUomId) {
        this.cUomId = cUomId;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name == null ? null : name.trim();
    }

    public String getValue() {
        return value;
    }

    public void setValue(String value) {
        this.value = value == null ? null : value.trim();
    }

    public Long getmSizegroupId() {
        return mSizegroupId;
    }

    public void setmSizegroupId(Long mSizegroupId) {
        this.mSizegroupId = mSizegroupId;
    }

    public Long getmAttributesetId() {
        return mAttributesetId;
    }

    public void setmAttributesetId(Long mAttributesetId) {
        this.mAttributesetId = mAttributesetId;
    }

    public String getImageurl() {
        return imageurl;
    }

    public void setImageurl(String imageurl) {
        this.imageurl = imageurl == null ? null : imageurl.trim();
    }

    public Long getmDim1Id() {
        return mDim1Id;
    }

    public void setmDim1Id(Long mDim1Id) {
        this.mDim1Id = mDim1Id;
    }

    public Long getmDim2Id() {
        return mDim2Id;
    }

    public void setmDim2Id(Long mDim2Id) {
        this.mDim2Id = mDim2Id;
    }

    public Long getmDim3Id() {
        return mDim3Id;
    }

    public void setmDim3Id(Long mDim3Id) {
        this.mDim3Id = mDim3Id;
    }

    public Long getmDim4Id() {
        return mDim4Id;
    }

    public void setmDim4Id(Long mDim4Id) {
        this.mDim4Id = mDim4Id;
    }

    public Long getmDim5Id() {
        return mDim5Id;
    }

    public void setmDim5Id(Long mDim5Id) {
        this.mDim5Id = mDim5Id;
    }

    public Long getmDim6Id() {
        return mDim6Id;
    }

    public void setmDim6Id(Long mDim6Id) {
        this.mDim6Id = mDim6Id;
    }

    public Long getmDim7Id() {
        return mDim7Id;
    }

    public void setmDim7Id(Long mDim7Id) {
        this.mDim7Id = mDim7Id;
    }

    public Long getmDim8Id() {
        return mDim8Id;
    }

    public void setmDim8Id(Long mDim8Id) {
        this.mDim8Id = mDim8Id;
    }

    public Long getmDim9Id() {
        return mDim9Id;
    }

    public void setmDim9Id(Long mDim9Id) {
        this.mDim9Id = mDim9Id;
    }

    public Long getmDim10Id() {
        return mDim10Id;
    }

    public void setmDim10Id(Long mDim10Id) {
        this.mDim10Id = mDim10Id;
    }

    public String getSerialno() {
        return serialno;
    }

    public void setSerialno(String serialno) {
        this.serialno = serialno == null ? null : serialno.trim();
    }

    public String getProarea() {
        return proarea;
    }

    public void setProarea(String proarea) {
        this.proarea = proarea == null ? null : proarea.trim();
    }

    public String getBatchno() {
        return batchno;
    }

    public void setBatchno(String batchno) {
        this.batchno = batchno == null ? null : batchno.trim();
    }

    public String getFlowno() {
        return flowno;
    }

    public void setFlowno(String flowno) {
        this.flowno = flowno == null ? null : flowno.trim();
    }

    public String getUnit() {
        return unit;
    }

    public void setUnit(String unit) {
        this.unit = unit == null ? null : unit.trim();
    }

    public String getDocumentnote() {
        return documentnote;
    }

    public void setDocumentnote(String documentnote) {
        this.documentnote = documentnote == null ? null : documentnote.trim();
    }

    public BigDecimal getPrecost() {
        return precost;
    }

    public void setPrecost(BigDecimal precost) {
        this.precost = precost;
    }

    public BigDecimal getPricelist() {
        return pricelist;
    }

    public void setPricelist(BigDecimal pricelist) {
        this.pricelist = pricelist;
    }

    public String getHelp() {
        return help;
    }

    public void setHelp(String help) {
        this.help = help == null ? null : help.trim();
    }

    public String getDescription() {
        return description;
    }

    public void setDescription(String description) {
        this.description = description == null ? null : description.trim();
    }

    public String getDescriptionurl() {
        return descriptionurl;
    }

    public void setDescriptionurl(String descriptionurl) {
        this.descriptionurl = descriptionurl == null ? null : descriptionurl.trim();
    }

    public String getVersionno() {
        return versionno;
    }

    public void setVersionno(String versionno) {
        this.versionno = versionno == null ? null : versionno.trim();
    }

    public Long getmDim11Id() {
        return mDim11Id;
    }

    public void setmDim11Id(Long mDim11Id) {
        this.mDim11Id = mDim11Id;
    }

    public Long getmDim12Id() {
        return mDim12Id;
    }

    public void setmDim12Id(Long mDim12Id) {
        this.mDim12Id = mDim12Id;
    }

    public String getIsfairpro() {
        return isfairpro;
    }

    public void setIsfairpro(String isfairpro) {
        this.isfairpro = isfairpro == null ? null : isfairpro.trim();
    }

    public Long getmDim13Id() {
        return mDim13Id;
    }

    public void setmDim13Id(Long mDim13Id) {
        this.mDim13Id = mDim13Id;
    }

    public Long getmDim14Id() {
        return mDim14Id;
    }

    public void setmDim14Id(Long mDim14Id) {
        this.mDim14Id = mDim14Id;
    }

    public Long getmDim15Id() {
        return mDim15Id;
    }

    public void setmDim15Id(Long mDim15Id) {
        this.mDim15Id = mDim15Id;
    }

    public String getFairpdttype() {
        return fairpdttype;
    }

    public void setFairpdttype(String fairpdttype) {
        this.fairpdttype = fairpdttype == null ? null : fairpdttype.trim();
    }

    public String getColors() {
        return colors;
    }

    public void setColors(String colors) {
        this.colors = colors == null ? null : colors.trim();
    }

    public String getSizes() {
        return sizes;
    }

    public void setSizes(String sizes) {
        this.sizes = sizes == null ? null : sizes.trim();
    }

    public String getIntscode() {
        return intscode;
    }

    public void setIntscode(String intscode) {
        this.intscode = intscode == null ? null : intscode.trim();
    }

    public Date getStatustime() {
        return statustime;
    }

    public void setStatustime(Date statustime) {
        this.statustime = statustime;
    }

    public Long getStatuserid() {
        return statuserid;
    }

    public void setStatuserid(Long statuserid) {
        this.statuserid = statuserid;
    }

    public Long getAuPiId() {
        return auPiId;
    }

    public void setAuPiId(Long auPiId) {
        this.auPiId = auPiId;
    }

    public String getAuState() {
        return auState;
    }

    public void setAuState(String auState) {
        this.auState = auState == null ? null : auState.trim();
    }

    public Long getmDim16Id() {
        return mDim16Id;
    }

    public void setmDim16Id(Long mDim16Id) {
        this.mDim16Id = mDim16Id;
    }

    public Long getmDim17Id() {
        return mDim17Id;
    }

    public void setmDim17Id(Long mDim17Id) {
        this.mDim17Id = mDim17Id;
    }

    public Long getmDim18Id() {
        return mDim18Id;
    }

    public void setmDim18Id(Long mDim18Id) {
        this.mDim18Id = mDim18Id;
    }

    public Long getmDim19Id() {
        return mDim19Id;
    }

    public void setmDim19Id(Long mDim19Id) {
        this.mDim19Id = mDim19Id;
    }

    public Long getmDim20Id() {
        return mDim20Id;
    }

    public void setmDim20Id(Long mDim20Id) {
        this.mDim20Id = mDim20Id;
    }

    public String getAddsdesc() {
        return addsdesc;
    }

    public void setAddsdesc(String addsdesc) {
        this.addsdesc = addsdesc == null ? null : addsdesc.trim();
    }

    public String getShortname() {
        return shortname;
    }

    public void setShortname(String shortname) {
        this.shortname = shortname == null ? null : shortname.trim();
    }

    public String getStepcode() {
        return stepcode;
    }

    public void setStepcode(String stepcode) {
        this.stepcode = stepcode == null ? null : stepcode.trim();
    }

    public String getDatasource() {
        return datasource;
    }

    public void setDatasource(String datasource) {
        this.datasource = datasource == null ? null : datasource.trim();
    }

    public String getFabcode() {
        return fabcode;
    }

    public void setFabcode(String fabcode) {
        this.fabcode = fabcode == null ? null : fabcode.trim();
    }

    public String getFabelement() {
        return fabelement;
    }

    public void setFabelement(String fabelement) {
        this.fabelement = fabelement == null ? null : fabelement.trim();
    }

    public BigDecimal getWeight() {
        return weight;
    }

    public void setWeight(BigDecimal weight) {
        this.weight = weight;
    }

    public String getGenerated() {
        return generated;
    }

    public void setGenerated(String generated) {
        this.generated = generated == null ? null : generated.trim();
    }

    public Short getStatus() {
        return status;
    }

    public void setStatus(Short status) {
        this.status = status;
    }

    public String getProtype() {
        return protype;
    }

    public void setProtype(String protype) {
        this.protype = protype == null ? null : protype.trim();
    }

    public String getMainfabremark() {
        return mainfabremark;
    }

    public void setMainfabremark(String mainfabremark) {
        this.mainfabremark = mainfabremark == null ? null : mainfabremark.trim();
    }

    public String getWithfabremark() {
        return withfabremark;
    }

    public void setWithfabremark(String withfabremark) {
        this.withfabremark = withfabremark == null ? null : withfabremark.trim();
    }

    public Long getyFactoryId() {
        return yFactoryId;
    }

    public void setyFactoryId(Long yFactoryId) {
        this.yFactoryId = yFactoryId;
    }

    public Integer getFabdate() {
        return fabdate;
    }

    public void setFabdate(Integer fabdate) {
        this.fabdate = fabdate;
    }

    public Integer getClothdate() {
        return clothdate;
    }

    public void setClothdate(Integer clothdate) {
        this.clothdate = clothdate;
    }

    public Integer getOtcdate() {
        return otcdate;
    }

    public void setOtcdate(Integer otcdate) {
        this.otcdate = otcdate;
    }

    public String getStyledescription() {
        return styledescription;
    }

    public void setStyledescription(String styledescription) {
        this.styledescription = styledescription == null ? null : styledescription.trim();
    }

    public String getBadicon() {
        return badicon;
    }

    public void setBadicon(String badicon) {
        this.badicon = badicon == null ? null : badicon.trim();
    }

    public String getCraftworkimg() {
        return craftworkimg;
    }

    public void setCraftworkimg(String craftworkimg) {
        this.craftworkimg = craftworkimg == null ? null : craftworkimg.trim();
    }

    public String getPackstyle() {
        return packstyle;
    }

    public void setPackstyle(String packstyle) {
        this.packstyle = packstyle == null ? null : packstyle.trim();
    }

    public String getTailornotice() {
        return tailornotice;
    }

    public void setTailornotice(String tailornotice) {
        this.tailornotice = tailornotice == null ? null : tailornotice.trim();
    }

    public String getMetricimg() {
        return metricimg;
    }

    public void setMetricimg(String metricimg) {
        this.metricimg = metricimg == null ? null : metricimg.trim();
    }

    public String getCraftworkimgtwo() {
        return craftworkimgtwo;
    }

    public void setCraftworkimgtwo(String craftworkimgtwo) {
        this.craftworkimgtwo = craftworkimgtwo == null ? null : craftworkimgtwo.trim();
    }

    public String getBathmode() {
        return bathmode;
    }

    public void setBathmode(String bathmode) {
        this.bathmode = bathmode == null ? null : bathmode.trim();
    }

    public Long getyStyleBomId() {
        return yStyleBomId;
    }

    public void setyStyleBomId(Long yStyleBomId) {
        this.yStyleBomId = yStyleBomId;
    }

    public String getCraftworkimgthree() {
        return craftworkimgthree;
    }

    public void setCraftworkimgthree(String craftworkimgthree) {
        this.craftworkimgthree = craftworkimgthree == null ? null : craftworkimgthree.trim();
    }

    public Long getyProofId() {
        return yProofId;
    }

    public void setyProofId(Long yProofId) {
        this.yProofId = yProofId;
    }

    public String getConfirmcomment() {
        return confirmcomment;
    }

    public void setConfirmcomment(String confirmcomment) {
        this.confirmcomment = confirmcomment == null ? null : confirmcomment.trim();
    }

    public BigDecimal getPricelist2() {
        return pricelist2;
    }

    public void setPricelist2(BigDecimal pricelist2) {
        this.pricelist2 = pricelist2;
    }

    public String getStandard() {
        return standard;
    }

    public void setStandard(String standard) {
        this.standard = standard == null ? null : standard.trim();
    }

    public String getComposition() {
        return composition;
    }

    public void setComposition(String composition) {
        this.composition = composition == null ? null : composition.trim();
    }

    public String getBatharea() {
        return batharea;
    }

    public void setBatharea(String batharea) {
        this.batharea = batharea == null ? null : batharea.trim();
    }

    public String getQuality() {
        return quality;
    }

    public void setQuality(String quality) {
        this.quality = quality == null ? null : quality.trim();
    }

    public Long getyBathmodeId1() {
        return yBathmodeId1;
    }

    public void setyBathmodeId1(Long yBathmodeId1) {
        this.yBathmodeId1 = yBathmodeId1;
    }

    public Long getyBathmodeId2() {
        return yBathmodeId2;
    }

    public void setyBathmodeId2(Long yBathmodeId2) {
        this.yBathmodeId2 = yBathmodeId2;
    }

    public Long getyBathmodeId3() {
        return yBathmodeId3;
    }

    public void setyBathmodeId3(Long yBathmodeId3) {
        this.yBathmodeId3 = yBathmodeId3;
    }

    public Long getyBathmodeId4() {
        return yBathmodeId4;
    }

    public void setyBathmodeId4(Long yBathmodeId4) {
        this.yBathmodeId4 = yBathmodeId4;
    }

    public Long getyBathmodeId5() {
        return yBathmodeId5;
    }

    public void setyBathmodeId5(Long yBathmodeId5) {
        this.yBathmodeId5 = yBathmodeId5;
    }

    public Long getyBathmodeId6() {
        return yBathmodeId6;
    }

    public void setyBathmodeId6(Long yBathmodeId6) {
        this.yBathmodeId6 = yBathmodeId6;
    }

    public Long getyBathmodeId7() {
        return yBathmodeId7;
    }

    public void setyBathmodeId7(Long yBathmodeId7) {
        this.yBathmodeId7 = yBathmodeId7;
    }

    public Long getSuggestmoq() {
        return suggestmoq;
    }

    public void setSuggestmoq(Long suggestmoq) {
        this.suggestmoq = suggestmoq;
    }

    public Long getbCodeFormatId() {
        return bCodeFormatId;
    }

    public void setbCodeFormatId(Long bCodeFormatId) {
        this.bCodeFormatId = bCodeFormatId;
    }

    public Long getBoxQty() {
        return boxQty;
    }

    public void setBoxQty(Long boxQty) {
        this.boxQty = boxQty;
    }

    public Long getcSupplierId() {
        return cSupplierId;
    }

    public void setcSupplierId(Long cSupplierId) {
        this.cSupplierId = cSupplierId;
    }

    public Long getmColorId() {
        return mColorId;
    }

    public void setmColorId(Long mColorId) {
        this.mColorId = mColorId;
    }

    public BigDecimal getAcost() {
        return acost;
    }

    public void setAcost(BigDecimal acost) {
        this.acost = acost;
    }

    public BigDecimal getPricedis() {
        return pricedis;
    }

    public void setPricedis(BigDecimal pricedis) {
        this.pricedis = pricedis;
    }

    public BigDecimal getPriceSug() {
        return priceSug;
    }

    public void setPriceSug(BigDecimal priceSug) {
        this.priceSug = priceSug;
    }

    public BigDecimal getOrigcost() {
        return origcost;
    }

    public void setOrigcost(BigDecimal origcost) {
        this.origcost = origcost;
    }

    public Short getTransstatus() {
        return transstatus;
    }

    public void setTransstatus(Short transstatus) {
        this.transstatus = transstatus;
    }

    public String geteName() {
        return eName;
    }

    public void seteName(String eName) {
        this.eName = eName == null ? null : eName.trim();
    }

    public Integer getMarketdate() {
        return marketdate;
    }

    public void setMarketdate(Integer marketdate) {
        this.marketdate = marketdate;
    }

    public String getIsspecial() {
        return isspecial;
    }

    public void setIsspecial(String isspecial) {
        this.isspecial = isspecial == null ? null : isspecial.trim();
    }

    public String getCategorymark() {
        return categorymark;
    }

    public void setCategorymark(String categorymark) {
        this.categorymark = categorymark == null ? null : categorymark.trim();
    }

    public String getCatalogno() {
        return catalogno;
    }

    public void setCatalogno(String catalogno) {
        this.catalogno = catalogno == null ? null : catalogno.trim();
    }

    public String getDescription2() {
        return description2;
    }

    public void setDescription2(String description2) {
        this.description2 = description2 == null ? null : description2.trim();
    }

    public String getClothes() {
        return clothes;
    }

    public void setClothes(String clothes) {
        this.clothes = clothes == null ? null : clothes.trim();
    }

    public String getIsinternal() {
        return isinternal;
    }

    public void setIsinternal(String isinternal) {
        this.isinternal = isinternal == null ? null : isinternal.trim();
    }

    public String getPdtPairup() {
        return pdtPairup;
    }

    public void setPdtPairup(String pdtPairup) {
        this.pdtPairup = pdtPairup == null ? null : pdtPairup.trim();
    }

    public Long getcCurrencyId() {
        return cCurrencyId;
    }

    public void setcCurrencyId(Long cCurrencyId) {
        this.cCurrencyId = cCurrencyId;
    }

    public BigDecimal getPricemember() {
        return pricemember;
    }

    public void setPricemember(BigDecimal pricemember) {
        this.pricemember = pricemember;
    }

    public String getMatchrule() {
        return matchrule;
    }

    public void setMatchrule(String matchrule) {
        this.matchrule = matchrule == null ? null : matchrule.trim();
    }

    public String getIsgift() {
        return isgift;
    }

    public void setIsgift(String isgift) {
        this.isgift = isgift == null ? null : isgift.trim();
    }

    public String getVirtualname() {
        return virtualname;
    }

    public void setVirtualname(String virtualname) {
        this.virtualname = virtualname == null ? null : virtualname.trim();
    }

    public BigDecimal getLowestDiscount() {
        return lowestDiscount;
    }

    public void setLowestDiscount(BigDecimal lowestDiscount) {
        this.lowestDiscount = lowestDiscount;
    }

    public Long getmDim21Id() {
        return mDim21Id;
    }

    public void setmDim21Id(Long mDim21Id) {
        this.mDim21Id = mDim21Id;
    }

    public String getIsagt() {
        return isagt;
    }

    public void setIsagt(String isagt) {
        this.isagt = isagt == null ? null : isagt.trim();
    }

    public Integer getAgtdate() {
        return agtdate;
    }

    public void setAgtdate(Integer agtdate) {
        this.agtdate = agtdate;
    }

    public String getImpStand() {
        return impStand;
    }

    public void setImpStand(String impStand) {
        this.impStand = impStand == null ? null : impStand.trim();
    }

    public String getColorsalias() {
        return colorsalias;
    }

    public void setColorsalias(String colorsalias) {
        this.colorsalias = colorsalias == null ? null : colorsalias.trim();
    }

    public BigDecimal getyPriceActual() {
        return yPriceActual;
    }

    public void setyPriceActual(BigDecimal yPriceActual) {
        this.yPriceActual = yPriceActual;
    }

    public String getIsteusclr() {
        return isteusclr;
    }

    public void setIsteusclr(String isteusclr) {
        this.isteusclr = isteusclr == null ? null : isteusclr.trim();
    }

    public String getSelfwork() {
        return selfwork;
    }

    public void setSelfwork(String selfwork) {
        this.selfwork = selfwork == null ? null : selfwork.trim();
    }

    public String getPdtType() {
        return pdtType;
    }

    public void setPdtType(String pdtType) {
        this.pdtType = pdtType == null ? null : pdtType.trim();
    }

    public Long getmDimNew1Id() {
        return mDimNew1Id;
    }

    public void setmDimNew1Id(Long mDimNew1Id) {
        this.mDimNew1Id = mDimNew1Id;
    }

    public Long getmDimNew2Id() {
        return mDimNew2Id;
    }

    public void setmDimNew2Id(Long mDimNew2Id) {
        this.mDimNew2Id = mDimNew2Id;
    }

    public Long getmDimNew3Id() {
        return mDimNew3Id;
    }

    public void setmDimNew3Id(Long mDimNew3Id) {
        this.mDimNew3Id = mDimNew3Id;
    }

    public Long getmDimNew4Id() {
        return mDimNew4Id;
    }

    public void setmDimNew4Id(Long mDimNew4Id) {
        this.mDimNew4Id = mDimNew4Id;
    }

    public Long getmDimNew5Id() {
        return mDimNew5Id;
    }

    public void setmDimNew5Id(Long mDimNew5Id) {
        this.mDimNew5Id = mDimNew5Id;
    }

    public Long getmDimNew6Id() {
        return mDimNew6Id;
    }

    public void setmDimNew6Id(Long mDimNew6Id) {
        this.mDimNew6Id = mDimNew6Id;
    }

    public Long getmDimNew7Id() {
        return mDimNew7Id;
    }

    public void setmDimNew7Id(Long mDimNew7Id) {
        this.mDimNew7Id = mDimNew7Id;
    }

    public Long getmDimNew8Id() {
        return mDimNew8Id;
    }

    public void setmDimNew8Id(Long mDimNew8Id) {
        this.mDimNew8Id = mDimNew8Id;
    }

    public Long getmDimNew9Id() {
        return mDimNew9Id;
    }

    public void setmDimNew9Id(Long mDimNew9Id) {
        this.mDimNew9Id = mDimNew9Id;
    }

    public Long getmDimNew10Id() {
        return mDimNew10Id;
    }

    public void setmDimNew10Id(Long mDimNew10Id) {
        this.mDimNew10Id = mDimNew10Id;
    }

    public Long getmDimNew11Id() {
        return mDimNew11Id;
    }

    public void setmDimNew11Id(Long mDimNew11Id) {
        this.mDimNew11Id = mDimNew11Id;
    }

    public Long getmDimNew12Id() {
        return mDimNew12Id;
    }

    public void setmDimNew12Id(Long mDimNew12Id) {
        this.mDimNew12Id = mDimNew12Id;
    }

    public Long getmDimNew13Id() {
        return mDimNew13Id;
    }

    public void setmDimNew13Id(Long mDimNew13Id) {
        this.mDimNew13Id = mDimNew13Id;
    }

    public Long getmDimNew14Id() {
        return mDimNew14Id;
    }

    public void setmDimNew14Id(Long mDimNew14Id) {
        this.mDimNew14Id = mDimNew14Id;
    }

    public Long getmDimNew15Id() {
        return mDimNew15Id;
    }

    public void setmDimNew15Id(Long mDimNew15Id) {
        this.mDimNew15Id = mDimNew15Id;
    }

    public Long getmDimNew16Id() {
        return mDimNew16Id;
    }

    public void setmDimNew16Id(Long mDimNew16Id) {
        this.mDimNew16Id = mDimNew16Id;
    }

    public Long getmDimNew17Id() {
        return mDimNew17Id;
    }

    public void setmDimNew17Id(Long mDimNew17Id) {
        this.mDimNew17Id = mDimNew17Id;
    }

    public Long getmDimNew18Id() {
        return mDimNew18Id;
    }

    public void setmDimNew18Id(Long mDimNew18Id) {
        this.mDimNew18Id = mDimNew18Id;
    }

    public Long getmDimNew19Id() {
        return mDimNew19Id;
    }

    public void setmDimNew19Id(Long mDimNew19Id) {
        this.mDimNew19Id = mDimNew19Id;
    }

    public Long getmDimNew20Id() {
        return mDimNew20Id;
    }

    public void setmDimNew20Id(Long mDimNew20Id) {
        this.mDimNew20Id = mDimNew20Id;
    }

    public Long getmDimNew21Id() {
        return mDimNew21Id;
    }

    public void setmDimNew21Id(Long mDimNew21Id) {
        this.mDimNew21Id = mDimNew21Id;
    }

    public Long getmDimNew22Id() {
        return mDimNew22Id;
    }

    public void setmDimNew22Id(Long mDimNew22Id) {
        this.mDimNew22Id = mDimNew22Id;
    }

    public Long getmDimNew23Id() {
        return mDimNew23Id;
    }

    public void setmDimNew23Id(Long mDimNew23Id) {
        this.mDimNew23Id = mDimNew23Id;
    }

    public Long getmDimNew24Id() {
        return mDimNew24Id;
    }

    public void setmDimNew24Id(Long mDimNew24Id) {
        this.mDimNew24Id = mDimNew24Id;
    }

    public Long getmDimNew25Id() {
        return mDimNew25Id;
    }

    public void setmDimNew25Id(Long mDimNew25Id) {
        this.mDimNew25Id = mDimNew25Id;
    }

    public Long getmDimNew26Id() {
        return mDimNew26Id;
    }

    public void setmDimNew26Id(Long mDimNew26Id) {
        this.mDimNew26Id = mDimNew26Id;
    }

    public Long getmDimNew27Id() {
        return mDimNew27Id;
    }

    public void setmDimNew27Id(Long mDimNew27Id) {
        this.mDimNew27Id = mDimNew27Id;
    }

    public Long getmDimNew28Id() {
        return mDimNew28Id;
    }

    public void setmDimNew28Id(Long mDimNew28Id) {
        this.mDimNew28Id = mDimNew28Id;
    }

    public Long getmDimNew29Id() {
        return mDimNew29Id;
    }

    public void setmDimNew29Id(Long mDimNew29Id) {
        this.mDimNew29Id = mDimNew29Id;
    }

    public Long getmDimNew30Id() {
        return mDimNew30Id;
    }

    public void setmDimNew30Id(Long mDimNew30Id) {
        this.mDimNew30Id = mDimNew30Id;
    }

    public Long getmDimNew31Id() {
        return mDimNew31Id;
    }

    public void setmDimNew31Id(Long mDimNew31Id) {
        this.mDimNew31Id = mDimNew31Id;
    }

    public Long getmDimNew32Id() {
        return mDimNew32Id;
    }

    public void setmDimNew32Id(Long mDimNew32Id) {
        this.mDimNew32Id = mDimNew32Id;
    }

    public Long getmDimNew33Id() {
        return mDimNew33Id;
    }

    public void setmDimNew33Id(Long mDimNew33Id) {
        this.mDimNew33Id = mDimNew33Id;
    }

    public Long getmDimNew34Id() {
        return mDimNew34Id;
    }

    public void setmDimNew34Id(Long mDimNew34Id) {
        this.mDimNew34Id = mDimNew34Id;
    }

    public Long getmDimNew35Id() {
        return mDimNew35Id;
    }

    public void setmDimNew35Id(Long mDimNew35Id) {
        this.mDimNew35Id = mDimNew35Id;
    }

    public Long getmDimNew36Id() {
        return mDimNew36Id;
    }

    public void setmDimNew36Id(Long mDimNew36Id) {
        this.mDimNew36Id = mDimNew36Id;
    }

    public Long getmDimNew37Id() {
        return mDimNew37Id;
    }

    public void setmDimNew37Id(Long mDimNew37Id) {
        this.mDimNew37Id = mDimNew37Id;
    }

    public Long getmDimNew38Id() {
        return mDimNew38Id;
    }

    public void setmDimNew38Id(Long mDimNew38Id) {
        this.mDimNew38Id = mDimNew38Id;
    }

    public Long getmDimNew39Id() {
        return mDimNew39Id;
    }

    public void setmDimNew39Id(Long mDimNew39Id) {
        this.mDimNew39Id = mDimNew39Id;
    }

    public Long getmDimNew40Id() {
        return mDimNew40Id;
    }

    public void setmDimNew40Id(Long mDimNew40Id) {
        this.mDimNew40Id = mDimNew40Id;
    }

    public Long getmDimNew41Id() {
        return mDimNew41Id;
    }

    public void setmDimNew41Id(Long mDimNew41Id) {
        this.mDimNew41Id = mDimNew41Id;
    }

    public String getXwValue() {
        return xwValue;
    }

    public void setXwValue(String xwValue) {
        this.xwValue = xwValue == null ? null : xwValue.trim();
    }

    public String getYcValue() {
        return ycValue;
    }

    public void setYcValue(String ycValue) {
        this.ycValue = ycValue == null ? null : ycValue.trim();
    }

    public BigDecimal getJkValue() {
        return jkValue;
    }

    public void setJkValue(BigDecimal jkValue) {
        this.jkValue = jkValue;
    }

    public BigDecimal getXcValue() {
        return xcValue;
    }

    public void setXcValue(BigDecimal xcValue) {
        this.xcValue = xcValue;
    }

    public BigDecimal getLbccValue() {
        return lbccValue;
    }

    public void setLbccValue(BigDecimal lbccValue) {
        this.lbccValue = lbccValue;
    }

    public BigDecimal getXbccValue() {
        return xbccValue;
    }

    public void setXbccValue(BigDecimal xbccValue) {
        this.xbccValue = xbccValue;
    }

    public BigDecimal getXkccValue() {
        return xkccValue;
    }

    public void setXkccValue(BigDecimal xkccValue) {
        this.xkccValue = xkccValue;
    }

    public String getDkValue() {
        return dkValue;
    }

    public void setDkValue(String dkValue) {
        this.dkValue = dkValue == null ? null : dkValue.trim();
    }

    public BigDecimal getDkslValue() {
        return dkslValue;
    }

    public void setDkslValue(BigDecimal dkslValue) {
        this.dkslValue = dkslValue;
    }

    public BigDecimal getHzxcValue() {
        return hzxcValue;
    }

    public void setHzxcValue(BigDecimal hzxcValue) {
        this.hzxcValue = hzxcValue;
    }

    public BigDecimal getDc1Value() {
        return dc1Value;
    }

    public void setDc1Value(BigDecimal dc1Value) {
        this.dc1Value = dc1Value;
    }

    public BigDecimal getDcValue() {
        return dcValue;
    }

    public void setDcValue(BigDecimal dcValue) {
        this.dcValue = dcValue;
    }

    public BigDecimal getMjkValue() {
        return mjkValue;
    }

    public void setMjkValue(BigDecimal mjkValue) {
        this.mjkValue = mjkValue;
    }

    public BigDecimal getMjcValue() {
        return mjcValue;
    }

    public void setMjcValue(BigDecimal mjcValue) {
        this.mjcValue = mjcValue;
    }

    public String getIsRetail() {
        return isRetail;
    }

    public void setIsRetail(String isRetail) {
        this.isRetail = isRetail == null ? null : isRetail.trim();
    }

    public String getNotlimitdis() {
        return notlimitdis;
    }

    public void setNotlimitdis(String notlimitdis) {
        this.notlimitdis = notlimitdis == null ? null : notlimitdis.trim();
    }

    public String getIsMscode() {
        return isMscode;
    }

    public void setIsMscode(String isMscode) {
        this.isMscode = isMscode == null ? null : isMscode.trim();
    }

    public String getColorarrDes() {
        return colorarrDes;
    }

    public void setColorarrDes(String colorarrDes) {
        this.colorarrDes = colorarrDes == null ? null : colorarrDes.trim();
    }

    public String getDkNumber() {
        return dkNumber;
    }

    public void setDkNumber(String dkNumber) {
        this.dkNumber = dkNumber == null ? null : dkNumber.trim();
    }

    public String getColorarr() {
        return colorarr;
    }

    public void setColorarr(String colorarr) {
        this.colorarr = colorarr == null ? null : colorarr.trim();
    }

    public Long getColorvalue() {
        return colorvalue;
    }

    public void setColorvalue(Long colorvalue) {
        this.colorvalue = colorvalue;
    }

    public String getAmjValue() {
        return amjValue;
    }

    public void setAmjValue(String amjValue) {
        this.amjValue = amjValue == null ? null : amjValue.trim();
    }

    public String getQtmjValue() {
        return qtmjValue;
    }

    public void setQtmjValue(String qtmjValue) {
        this.qtmjValue = qtmjValue == null ? null : qtmjValue.trim();
    }

    public BigDecimal getHzxc1Value() {
        return hzxc1Value;
    }

    public void setHzxc1Value(BigDecimal hzxc1Value) {
        this.hzxc1Value = hzxc1Value;
    }

    public String getQtxkValue() {
        return qtxkValue;
    }

    public void setQtxkValue(String qtxkValue) {
        this.qtxkValue = qtxkValue == null ? null : qtxkValue.trim();
    }

    public String getJzValue() {
        return jzValue;
    }

    public void setJzValue(String jzValue) {
        this.jzValue = jzValue == null ? null : jzValue.trim();
    }

    public String getQtxbValue() {
        return qtxbValue;
    }

    public void setQtxbValue(String qtxbValue) {
        this.qtxbValue = qtxbValue == null ? null : qtxbValue.trim();
    }

    public String getTangz() {
        return tangz;
    }

    public void setTangz(String tangz) {
        this.tangz = tangz == null ? null : tangz.trim();
    }

    public String getHuax() {
        return huax;
    }

    public void setHuax(String huax) {
        this.huax = huax == null ? null : huax.trim();
    }

    public String getLxsx2Value() {
        return lxsx2Value;
    }

    public void setLxsx2Value(String lxsx2Value) {
        this.lxsx2Value = lxsx2Value == null ? null : lxsx2Value.trim();
    }

    public String getLxsx1Value() {
        return lxsx1Value;
    }

    public void setLxsx1Value(String lxsx1Value) {
        this.lxsx1Value = lxsx1Value == null ? null : lxsx1Value.trim();
    }

    public String getQtlbValue() {
        return qtlbValue;
    }

    public void setQtlbValue(String qtlbValue) {
        this.qtlbValue = qtlbValue == null ? null : qtlbValue.trim();
    }

    public String getLxsx3Value() {
        return lxsx3Value;
    }

    public void setLxsx3Value(String lxsx3Value) {
        this.lxsx3Value = lxsx3Value == null ? null : lxsx3Value.trim();
    }

    public String getLxsx4Value() {
        return lxsx4Value;
    }

    public void setLxsx4Value(String lxsx4Value) {
        this.lxsx4Value = lxsx4Value == null ? null : lxsx4Value.trim();
    }

    public String getLxsx5Value() {
        return lxsx5Value;
    }

    public void setLxsx5Value(String lxsx5Value) {
        this.lxsx5Value = lxsx5Value == null ? null : lxsx5Value.trim();
    }

    public String getLingNumber2Value() {
        return lingNumber2Value;
    }

    public void setLingNumber2Value(String lingNumber2Value) {
        this.lingNumber2Value = lingNumber2Value == null ? null : lingNumber2Value.trim();
    }

    public String getLingNumber1Value() {
        return lingNumber1Value;
    }

    public void setLingNumber1Value(String lingNumber1Value) {
        this.lingNumber1Value = lingNumber1Value == null ? null : lingNumber1Value.trim();
    }

    public String getLingValue() {
        return lingValue;
    }

    public void setLingValue(String lingValue) {
        this.lingValue = lingValue == null ? null : lingValue.trim();
    }

    public String getQtxbzzValue() {
        return qtxbzzValue;
    }

    public void setQtxbzzValue(String qtxbzzValue) {
        this.qtxbzzValue = qtxbzzValue == null ? null : qtxbzzValue.trim();
    }

    public BigDecimal getXbcValue() {
        return xbcValue;
    }

    public void setXbcValue(BigDecimal xbcValue) {
        this.xbcValue = xbcValue;
    }

    public BigDecimal getLingCcValue() {
        return lingCcValue;
    }

    public void setLingCcValue(BigDecimal lingCcValue) {
        this.lingCcValue = lingCcValue;
    }

    public String getXbzzValue() {
        return xbzzValue;
    }

    public void setXbzzValue(String xbzzValue) {
        this.xbzzValue = xbzzValue == null ? null : xbzzValue.trim();
    }

    public String getQtjxValue() {
        return qtjxValue;
    }

    public void setQtjxValue(String qtjxValue) {
        this.qtjxValue = qtjxValue == null ? null : qtjxValue.trim();
    }

    public String getQtxxValue() {
        return qtxxValue;
    }

    public void setQtxxValue(String qtxxValue) {
        this.qtxxValue = qtxxValue == null ? null : qtxxValue.trim();
    }

    public String getXbValue() {
        return xbValue;
    }

    public void setXbValue(String xbValue) {
        this.xbValue = xbValue == null ? null : xbValue.trim();
    }

    public String getSizeType() {
        return sizeType;
    }

    public void setSizeType(String sizeType) {
        this.sizeType = sizeType == null ? null : sizeType.trim();
    }

    public BigDecimal getPrice() {
        return price;
    }

    public void setPrice(BigDecimal price) {
        this.price = price;
    }

    public BigDecimal getPrice1() {
        return price1;
    }

    public void setPrice1(BigDecimal price1) {
        this.price1 = price1;
    }

    public BigDecimal getJlPrice() {
        return jlPrice;
    }

    public void setJlPrice(BigDecimal jlPrice) {
        this.jlPrice = jlPrice;
    }

    public Long getApproveId() {
        return approveId;
    }

    public void setApproveId(Long approveId) {
        this.approveId = approveId;
    }

    public BigDecimal getRetailPrice() {
        return retailPrice;
    }

    public void setRetailPrice(BigDecimal retailPrice) {
        this.retailPrice = retailPrice;
    }

    public BigDecimal getRetailPrice1() {
        return retailPrice1;
    }

    public void setRetailPrice1(BigDecimal retailPrice1) {
        this.retailPrice1 = retailPrice1;
    }

    public String getMptbelongtype() {
        return mptbelongtype;
    }

    public void setMptbelongtype(String mptbelongtype) {
        this.mptbelongtype = mptbelongtype == null ? null : mptbelongtype.trim();
    }

    public Long getmBasemptsizeId() {
        return mBasemptsizeId;
    }

    public void setmBasemptsizeId(Long mBasemptsizeId) {
        this.mBasemptsizeId = mBasemptsizeId;
    }

    public String getXwoldValue() {
        return xwoldValue;
    }

    public void setXwoldValue(String xwoldValue) {
        this.xwoldValue = xwoldValue == null ? null : xwoldValue.trim();
    }

    public String getYcoldValue() {
        return ycoldValue;
    }

    public void setYcoldValue(String ycoldValue) {
        this.ycoldValue = ycoldValue == null ? null : ycoldValue.trim();
    }

    public String getJkoldValue() {
        return jkoldValue;
    }

    public void setJkoldValue(String jkoldValue) {
        this.jkoldValue = jkoldValue == null ? null : jkoldValue.trim();
    }

    public String getXcoldValue() {
        return xcoldValue;
    }

    public void setXcoldValue(String xcoldValue) {
        this.xcoldValue = xcoldValue == null ? null : xcoldValue.trim();
    }

    public String getShowpic() {
        return showpic;
    }

    public void setShowpic(String showpic) {
        this.showpic = showpic == null ? null : showpic.trim();
    }

    public String getCzDate() {
        return czDate;
    }

    public void setCzDate(String czDate) {
        this.czDate = czDate == null ? null : czDate.trim();
    }

    public String getNo() {
        return no;
    }

    public void setNo(String no) {
        this.no = no == null ? null : no.trim();
    }

    public String getCustomer() {
        return customer;
    }

    public void setCustomer(String customer) {
        this.customer = customer == null ? null : customer.trim();
    }

    public String getDescription3() {
        return description3;
    }

    public void setDescription3(String description3) {
        this.description3 = description3 == null ? null : description3.trim();
    }

    public String getDescription5() {
        return description5;
    }

    public void setDescription5(String description5) {
        this.description5 = description5 == null ? null : description5.trim();
    }

    public String getDescription4() {
        return description4;
    }

    public void setDescription4(String description4) {
        this.description4 = description4 == null ? null : description4.trim();
    }

    public String getDescription6() {
        return description6;
    }

    public void setDescription6(String description6) {
        this.description6 = description6 == null ? null : description6.trim();
    }

    public String getDescription7() {
        return description7;
    }

    public void setDescription7(String description7) {
        this.description7 = description7 == null ? null : description7.trim();
    }

    public Long getcCustomerId1() {
        return cCustomerId1;
    }

    public void setcCustomerId1(Long cCustomerId1) {
        this.cCustomerId1 = cCustomerId1;
    }

    public Long getmDim22Id() {
        return mDim22Id;
    }

    public void setmDim22Id(Long mDim22Id) {
        this.mDim22Id = mDim22Id;
    }

    public Long getToStoreid() {
        return toStoreid;
    }

    public void setToStoreid(Long toStoreid) {
        this.toStoreid = toStoreid;
    }

    public BigDecimal getActualprice() {
        return actualprice;
    }

    public void setActualprice(BigDecimal actualprice) {
        this.actualprice = actualprice;
    }

    public String getIsRecommend() {
        return isRecommend;
    }

    public void setIsRecommend(String isRecommend) {
        this.isRecommend = isRecommend;
    }
}